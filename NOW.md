...
- GitHub Actions release attempt `26382941970` failed in `Verify code before release` because CI installed from `pyproject.toml` and `aiosqlite` was missing. After adding it, local release-gate checks pass: `ruff check src scripts tests`; `pytest tests -q -m "not slow"` (139 passed, 2 deselected); elevated `npm run test:desktop`.
