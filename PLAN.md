...
- Added MCP-native runnable backend: run payloads now carry `browser_backend` and `mcp_page`; preflight accepts an MCP-selected tab as the work window without `cdp_url`; `AgentBrain` routes `browser_backend="mcp"` to a new minimal `McpBrowserExecutor` instead of browser-use/CDP.
- The MCP executor keeps a helper-owned persistent Chrome DevTools MCP session, selects the existing Chrome tab, snapshots it, asks the model/planner for strict one-action JSON, executes only a narrow allowlist (`take_snapshot`, `click`, `fill`, `type_text`, `press_key`, `wait_for` plus semantic `ask_user`, `decision_checkpoint`, `report_detection`, `report_outcome`), refreshes snapshots after browser actions, writes a bounded MCP artifact, and returns a normal `TaskResult`.
- Dashboard MCP-selected tabs now count as runnable through `browser_backend:"mcp"` even without CDP handoff. Start/HUCA skip CDP `/browser/status` checks in MCP mode and send `mcp_page` to the helper.
- Verification for MCP-native implementation passed: focused MCP/daemon/run orchestration tests (`pytest tests/unit/test_mcp_executor.py tests/unit/test_run_orchestration.py tests/unit/test_daemon_server.py -q`, 30 passed), JavaScript syntax/protocol checks, `npm run test:dashboard`, full unit suite (`132 passed`), `ruff check src tests`, and `git diff --check`.
- Minimal live MCP-native smoke passed: `list_pages` returned 7 existing Chrome tabs, a safe local CLIProxy tab was selected, the MCP executor ran `take_snapshot` then `report_outcome` without clicking/typing, returned `status: success`, and wrote `/tmp/flyingpig-mcp-smoke/mcp_session_20260706_041301.json`.
