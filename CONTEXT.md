...
- Before the first successful run, show Configure -> Open website -> Start as three sequential screens, never simultaneously. Step 1 owns model setup, Step 2 owns work-window launch and website preparation, and Step 3 owns the request form. After `first_run_started`, repeat users go directly to the request form. If a previously configured model becomes invalid, Step 1 fully replaces the page with recovery wording.
